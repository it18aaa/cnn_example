...
conv1 = keras.layers.Conv2D(
    32, kernel_size=KERNEL_SIZE, activation='relu', padding='same')(inp)
pool1 = keras.layers.MaxPooling2D(pool_size=(2, 2))(conv1)
norm1 = keras.layers.BatchNormalization(axis=-1)(pool1)
drop1 = keras.layers.Dropout(rate=0.2)(norm1)

conv2 = keras.layers.Conv2D(
    32, kernel_size=KERNEL_SIZE, activation='relu', padding='same')(drop1)
...
out = keras.layers.Dense(2, activation='sigmoid')(drop4)   # 2 categories
...
X_train, X_test, y_train, y_test = train_test_split(
    dataset, to_categorical(np.array(label)), test_size=0.20, random_state=0)
